"""
Módulo Gemini Vision para a VPS.
Usado para processar prints enviados pelos clientes via celular.
Não usa PyTorch, EasyOCR ou OpenCV — apenas chamada HTTP à API do Google.
"""
import os
import re
import json
...
async def analyze_auction_frame(image_base64: str, api_key: Optional[str] = None) -> Dict[str, Any]:
    ...
    if not key:
        return {
            'success': False,
            'error': 'GEMINI_API_KEY não configurada no servidor VPS.'
        }
    ...
    try:
        ...
        if response.status_code != 200:
            return {
                'success': False,
                'error': f'Erro Gemini API ({response.status_code}): {response.text[:200]}'
            }

        data = response.json()
        candidates = data.get('candidates', [])
        if not candidates:
            return {'success': False, 'error': 'Nenhuma resposta retornada pelo Gemini Vision.'}

        parts = candidates[0].get('content', {}).get('parts', [])
        if not parts:
            return {'success': False, 'error': 'Conteúdo vazio retornado pelo Gemini Vision.'}
        ...
        return {'success': False, 'error': f'Erro ao decodificar JSON do Gemini: {str(je)}'}
    except Exception as e:
        return {'success': False, 'error': f'Exceção ao comunicar com Gemini Vision: {str(e)}'}
